...
import (
	...
	"encoding/base64"
	"errors"
	"github.com/saveio/themis/crypto/ristretto/internal/edwards25519"
	"github.com/saveio/themis/crypto/ristretto/internal/radix51"
	"github.com/saveio/themis/crypto/ristretto/internal/scalar"
)
...
func (e *Element) MultiScalarMult_hmy(s []*Scalar, p []*Element) *Element {
	if len(p) != len(s) {
		panic("ristretto255: MultiScalarMult invoked with mismatched slice lengths")
	}
	points := make([]*edwards25519.ProjP3, len(p))
	...
	e.r.MultiscalarMul_hmy(scalars, points)
	return e
}
...
func (e *Element) VarTimeMultiScalarMult(s []*Scalar, p []*Element) *Element {
	...
	e.r.VartimeMultiscalarMul_hmy(scalars, points)
	return e
}
