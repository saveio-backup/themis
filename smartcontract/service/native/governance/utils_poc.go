...
func putPeriodSummary(native *native.NativeService, period uint32, periodSummary *PeriodSummary) error {
	contract := native.ContextRef.CurrentContext().ContractAddress
	key := GenPeriodSummaryKey(contract, period)

	bf := new(bytes.Buffer)
	if err := periodSummary.Serialize(bf); err != nil {
		return fmt.Errorf("serialize, serialize periodSummary error: %v", err)
	}
...
// miner info during mining period
func GenPeriodInfosKey(contract common.Address, address common.Address) []byte {
	key := append(contract[:], []byte(PERIOD_INFOS)...)
	key = append(key, address[:]...)
	return key

}

func getPeriodInfos(native *native.NativeService, address common.Address) (*PeriodInfos, error) {
	contract := native.ContextRef.CurrentContext().ContractAddress
	key := GenPeriodInfosKey(contract, address)
	periodInfosBytes, err := utils.GetStorageItem(native, key)
	if err != nil {
		return nil, fmt.Errorf("get periodInfosBytes error: %v", err)
	}

	periodInfos := &PeriodInfos{Address: address}

	if periodInfosBytes != nil {
		if err := periodInfos.Deserialize(bytes.NewBuffer(periodInfosBytes.Value)); err != nil {
			return nil, fmt.Errorf("deserialize, deserialize periodInfos error: %v", err)
		}
	}
	return periodInfos, nil
}

func putPeriodsInfo(native *native.NativeService, address common.Address, periodInfos *PeriodInfos) error {
	contract := native.ContextRef.CurrentContext().ContractAddress
	key := GenPeriodInfosKey(contract, address)

	bf := new(bytes.Buffer)
	if err := periodInfos.Serialize(bf); err != nil {
		return fmt.Errorf("serialize, serialize periodInfos error: %v", err)
	}
...
func PledgeViewInMiningPeriod(view uint32) bool {
	period := GetMiningPeriod(view)
	return (view - (period-1)*NUM_VIEW_PER_PERIOD) == NUM_VIEW_PER_VERIFY_PERIOD
}

func triggerPledgeForPeriod(native *native.NativeService, view uint32) error {
	if !PledgeViewInMiningPeriod(view) {
		return nil
	}
	contract := native.ContextRef.CurrentContext().ContractAddress

	iter := native.CacheDB.NewIterator(utils.ConcatKey(contract, []byte(PERIOD_INFOS)))
	defer iter.Release()
	for has := iter.First(); has; has = iter.Next() {
		periodInfosStore, err := cstates.GetValueFromRawStorageItem(iter.Value())
		if err != nil {
			return fmt.Errorf("periodInfosStore is not available!:%v", err)
		}
		periodInfos := &PeriodInfos{}

		if err := periodInfos.Deserialize(bytes.NewBuffer(periodInfosStore)); err != nil {
			return fmt.Errorf("deserialize, deserialize periodInfos error: %v", err)
		}

		plotSize := periodInfos.curPeriod.PlotSize
		if plotSize > 0 {
			log.Debugf("triggerPledgeForPeriod: plot size calculated for %s is %d MB", periodInfos.Address.ToBase58(), plotSize)

			ensure := &ensureSpaceForMiningEvent{
				MiningPeriod: GetMiningPeriod(view),
				PlotSize:     plotSize,
				Address:      periodInfos.Address.ToBase58(),
			}
			EnsureSpaceForMiningEvent(native, ensure)
		}
...
func updatePeriod(native *native.NativeService, view uint32) error {
	if !IsLastViewInMiningPeriod(view) {
		return nil
	}
	contract := native.ContextRef.CurrentContext().ContractAddress

	//update pre/cur period
	iter := native.CacheDB.NewIterator(utils.ConcatKey(contract, []byte(PERIOD_INFOS)))
	defer iter.Release()
	for has := iter.First(); has; has = iter.Next() {
		periodInfosStore, err := cstates.GetValueFromRawStorageItem(iter.Value())
		...
		log.Debugf("updatePeriod,  miner %s detail %v", periodInfos.Address.ToBase58(), periodInfos)
		// adjust period info

		if periodInfos.curPeriod.Period != 0 || periodInfos.curPeriod.PlotSize != 0 {
			periodInfos.prePeriod = periodInfos.curPeriod
			periodInfos.curPeriod.Period = 0
			periodInfos.curPeriod.PlotSize = 0
		}

		err = putPeriodsInfo(native, periodInfos.Address, periodInfos)
		if err != nil {
			return fmt.Errorf("putPeriodsInfo, put PeriodInfos error: %v", err)
		}
	}
	if err := iter.Error(); err != nil {
		return err
