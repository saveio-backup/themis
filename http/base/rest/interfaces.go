...
import (
 ...
 func GetSmartCodeEventByHeightAndAddress(cmd map[string]interface{}) map[string]interface{} {
	 resp := ResponsePack(berr.SUCCESS)
 ...
 func GetSmartCodeEventByEventId(cmd map[string]interface{}) map[string]interface{} {
	 ...
	 addr, ok := cmd["ContractAddr"].(string)
	 if !ok {
		 return ResponsePack(berr.INVALID_PARAMS)
	 }
 ...
 func GetSmartCodeEventByEventIdAndHeights(cmd map[string]interface{}) map[string]interface{} {
	 resp := ResponsePack(berr.SUCCESS)
	 ...
	 start, _ := cmd["StartHeight"].(string)
	 startHeight := uint32(0)
	 if len(start) != 0 {
		 height, err := strconv.ParseInt(start, 10, 64)
		 if err != nil {
			 return ResponsePack(berr.INVALID_PARAMS)
		 }
	 ...
	 end, _ := cmd["EndHeight"].(string)
	 endHeight := uint32(0)
	 if len(end) != 0 {
		 height, err := strconv.ParseInt(end, 10, 64)
		 if err != nil {
			 return ResponsePack(berr.INVALID_PARAMS)
		 }
 ...
 func GetSysStatusScore(cmd map[string]interface{}) map[string]interface{} {
	 resp := ResponsePack(berr.SUCCESS)
	 resp["Result"] = sys.SysScore
	 return resp
 }
