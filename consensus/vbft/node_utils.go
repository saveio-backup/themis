...
func (self *Server) updateTimerParams(config *vconfig.ChainConfig) {
	...
	// atomic.StoreInt64(&endorseBlockTimeout, int64(config.HashMsgDelay))
	// atomic.StoreInt64(&commitBlockTimeout, int64(config.HashMsgDelay))
	...
	atomic.StoreInt64(&zeroTxBlockTimeout, int64(config.BlockMsgDelay))
}
